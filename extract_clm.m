load('ind0.dat');
for i = 1:9504
    filename = ['SA_005.out.clm_output.' num2str([i],'%05d') '.txt'];
    fid = fopen(filename);
    data = fscanf(fid, ['%f']);
    data(1:3,:) = [];
    data(ind0,:) = [];
    ...
    E = data((1601*4+1):1601*5);
    load('river_ind.dat');
    total_E(i,1) = 3600*(mean(E));
    E(river_ind) = 0;
    total_E(i,2) = 3600*(mean(E));
    ...
    T = data((1601*8+1):1601*9);
    vegetation_T(i,1) = 3600*(mean(T));
    T(river_ind) = 0;
    vegetation_T(:,2) = 3600*(mean(T));
